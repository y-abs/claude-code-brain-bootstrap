...
2. **`CLAUDE.md` — Fill ALL commented sections + scan for Critical Patterns**:
   ...
   - `<!-- {{CRITICAL_PATTERNS}} -->` — **DEPTH PROTOCOL**: run ALL 8 greps below. For each that returns results, you MUST create a domain doc (item 3) AND write the actual pattern from that file into `{{CRITICAL_PATTERNS}}`. Generic advice is worthless; project-specific patterns with real file paths are the goal.
     ...
     grep -rl 'setTopic\|KafkaConsumer\|KafkaProducer\|producer\.\|RabbitMQ\|SQSClient' . --include='*.js' --include='*.ts' 2>/dev/null | head -5 || true
     ...
     grep -rl 'adapter\|Adapter\|adapterFactory\|standardAdapter\|sandboxAdapter\|integration\|ExternalAPI' . --include='*.js' --include='*.ts' 2>/dev/null | head -5 || true
     ...
     # User onboarding / registration / KYC → claude/enrollment.md
     grep -rl 'enrollment\|Enrollment\|onboard\|kyc\|KYC\|registration.*flow\|stepper\|consent\|verify.*identity' . --include='*.js' --include='*.ts' 2>/dev/null | head -5 || true
     ```
...
5b. **Refine `.github/instructions/` globs** — `generate-copilot-docs.sh` creates instruction files with heuristic globs (e.g., `**/webhook*/**`). Review each and replace with **actual project paths** (e.g., `"core/iop-operator-notifier/**,core/packages/iop-adapters/**"`). Precise globs = instructions load only when truly relevant.
