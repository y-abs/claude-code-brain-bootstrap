---
model: haiku
description: Remove all git worktrees for merged branches — /clean-worktrees [--dry-run]
...
- Only removes branches merged into the default branch (`main`/`master`)
- Auto-detects default branch name via `git symbolic-ref`
- Skips `main` and `master` (always protected)
